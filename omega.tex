...
Two level schedulers manages to give every application, also called
framework, the ability to schedule its tasks independently while
sharing all the resources of the cluster between the frameworks. This
is achieved with a model based on resource sharing where a master runs a set of frameworks. The master
provides the possibility to use the available resources to the frameworks
...
- How does mesos manage to fulfil per-framework constraints when the
master doesn't have a knowledge of the framework needs? Through offer
rejection and delay scheduling
